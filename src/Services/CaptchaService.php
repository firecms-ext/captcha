...
class CaptchaService implements CaptchaServiceInterface
{
    ...
    public function create(array $generator): string
    {
        // 图片宽(px)
    ...
    public function api(array $options = []): array
    {
        ...
        return [
            'key' => $generator['key'],
            'img' => 'data:image/jpeg;base64,' . base64_encode($this->create($generator)),
        ];
    }
    ...
    public function output(): string
    {
        ...
        // 记住验证码
        $this->session->set('captcha.key', $generator['key']);

        return $this->create($generator);
    }
    ...
    public function check(string $code, ?string $key = null): bool
    {
        ...
        $datetime = time();
        $bool = $data
            && isset($data['expired_at'], $data['hash'])
            && $data['expired_at'] >= $datetime
            && password_verify($code, $data['hash']);
    ...
    protected function generate(array $options = []): array
    {
        ...
        if ($this->math) {
            ...
            $characters = $this->useZh ? preg_split('/(?<!^)(?!$)/u', $this->zhSet) : str_split($this->codeSet);
